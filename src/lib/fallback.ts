import type { PitchPlan, PitchRequest, RepoContext } from "@/lib/types";
...
export function fallbackPitchPlan(
  ...
  const scenes = [
    ...
    },
    {
      ...
      narration: "Gemini reads the product surface and produces a tight story. VideoDB generates supporting clips and music while the renderer turns the script into a voice-led pitch video.",
  ...
  return {
    ...
    strategy:
      "Lead with the cost of unstructured demos, reveal the repo-to-pitch workflow quickly, then use VideoDB-generated media and a confident close.",
    score: 78,
    cta: "Generate the narrated browser preview, then finalize the VideoDB stream once generated assets are ready.",
    insights: [
